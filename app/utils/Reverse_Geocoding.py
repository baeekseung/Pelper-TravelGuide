...
def extract_clean_address(geocoding_result: dict) -> dict:
    ...
    for result in results:
        ...
        if name == "roadaddr":
            ...
            if road_name and building_number:
                road_address += f" {road_name} {building_number}"
                if building_sub:
                    road_address += f"-{building_sub}"
            ...
            if land_number1:
                land_address += f" {land_number1}"
                if land_number2:
                    land_address += f"-{land_number2}"

            address_info["land_address"] = {
                "full_address": land_address,
                "land_type": land_type,
    ...
    if "road_address" in address_info:
        ...
        address_info["main_address"] = address_info["land_address"]["full_address"]
    else:
        admin = address_info.get("administrative", {})
        address_info["main_address"] = (
            f"{admin.get('area1', '')} {admin.get('area2', '')} {admin.get('area3', '')}".strip()
        )
